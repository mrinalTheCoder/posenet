...
export type StringTriple = Triple<string>;
...
export const partIds = partNames.reduce((result: NumberDict, jointName, i): NumberDict => {
    result[jointName] = i;
    return result;
}, {}) as NumberDict;

export const symPartIds = symPartNames.reduce((result: NumberDict, jointName, i): NumberDict => {
    result[jointName] = i;
    return result;
}, {}) as NumberDict;
...
const connectedSymPartNames: StringTuple[] = [
    ['rightWrist', 'rightElbow'], ['rightElbow', 'rightShoulder'],
    ['rightShoulder', 'rightHip'], ['rightHip', 'rightKnee'],
    ['rightKnee', 'rightAnkle']
];

const jointIndentifiers: StringTriple[] = [
...
export const symPartIndices = connectedSymPartNames.map(
    ([jointNameA, jointNameB]) => ([partIds[jointNameA], partIds[jointNameB]])
);

export const angleIndices = jointIndentifiers.map(
    ([jointA, jointB, jointC]) => ([partIds[jointA], partIds[jointB], partIds[jointC]])
);
