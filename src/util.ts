...
import {connectedPartIndices, angleIndices, symPartIndices} from './keypoints';
...
export function getSymKeypoints(keypoints: Keypoint[], minConfidence: number): Keypoint[][] {
    return symPartIndices.reduce((result: Keypoint[][], [leftJoint, rightJoint]): Keypoint[][] => {
        if (eitherPointDoesntMeetConfidence(keypoints[leftJoint].score, keypoints[rightJoint].score, minConfidence)) {
            return result;
        }
        result.push([keypoints[leftJoint], keypoints[rightJoint]]);
...
export function getAngleKeyPoints(keypoints: Keypoint[], minConfidence: number): Keypoint [][] {
    return angleIndices.reduce((result: Keypoint[][], [jointA, jointB, jointC]): Keypoint[][] => {
        if (eitherPointDoesntMeetConfidence(keypoints[jointA].score, keypoints[jointB].score, minConfidence) || keypoints[jointC].score < minConfidence) {
            return result;
        }
...
export function getInputTensorDimensions(inputVal: PosenetInput): [number, number] {
  return (inputVal instanceof tf.Tensor) ? [inputVal.shape[0], inputVal.shape[1]] : [inputVal.height, inputVal.width];
}
